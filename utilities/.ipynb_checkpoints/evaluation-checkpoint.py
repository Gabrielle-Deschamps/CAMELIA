...
comm_indices = ['richness','cwm','fdis','fskew','fkurt']
...
def evaluate_prediction(plant_model, data_train, data_test, splist, tlist, T_scaled, method='raw', do_residual=False, eps=1e-6):
    ...
    if method=='th05':
        species_perfs = eval_species_perfs(Ys_hat_test, Ys_true_test, th_list=0.5)
        rich, cwm, fdis, _, _ = compute_functional_indices((Ys_hat_test>0.5)*1,T_scaled)
        prediction = (Ys_hat_test>0.5)*1
        pred_test = {
            'species': (Ys_hat_test>0.5)*1,
            'cwm': cwm,
            'fdis': fdis,
            'richness': rich
        }
        ...
        try:
            comm_perfs = plot_community_pred(obs_test,pred_test,tlist,algo='mtnn_th05',plot_residual=do_residual)
        ...
        y_pred = (Ys_hat_test > 0.5).values
        scores_par_ligne = [jaccard_score(y_true[i], y_pred[i]) for i in range(len(y_true))]
        jaccard_perfs.append(scores_par_ligne)
    ...
    if method=='opth':
        ...
        rich, cwm, fdis, _, _ = compute_functional_indices((Ys_hat_test>th_list)*1,T_scaled)
        ...
        try:
            ...
            comm_perfs = None
        y_true = Ys_true_test.values
        y_pred = (Ys_hat_test>th_list).values
        scores_par_ligne = [jaccard_score(y_true[i], y_pred[i]) for i in range(len(y_true))]
        jaccard_perfs.append(scores_par_ligne)
    
    if method=='opth_norm':
        th_list = optimize_thres(Ys_true_train,Ys_hat_train)
        Ys_hat = (Ys_hat_test - np.array(th_list))/(1 - np.array(th_list))
        Ys_hat = Ys_hat.clip(lower=0,upper=1)
        
        species_perfs = eval_species_perfs(Ys_hat, Ys_true_test, th_list=eps)
        rich, cwm, fdis, _, _ = compute_functional_indices((Ys_hat>eps)*1,T_scaled)
        prediction = (Ys_hat>eps)*1
        pred_test = {
            'species': (Ys_hat>eps)*1,
            'cwm': cwm,
            'fdis': fdis,
            'richness': rich
        }
        ...
        try:
            comm_perfs = plot_community_pred(obs_test,pred_test,tlist,algo='mtnn_opth_norm',plot_residual=do_residual)
            comm_perfs['jaccard']=jaccard_score(y_true=obs_test['species'],y_pred=pred_test['species'], average='micro')
        except:
            comm_perfs = None
        y_true = Ys_true_test.values
        y_pred = (Ys_hat>eps).values
        scores_par_ligne = [jaccard_score(y_true[i], y_pred[i]) for i in range(len(y_true))]
        jaccard_perfs.append(scores_par_ligne)
        
    if method=='prr':
        filter_prr = (Ys_hat_test.rank(axis=1,method='min',ascending=False)<=data_test_in[1 ])*1
        ...
        try:
            comm_perfs = plot_community_pred(obs_test,pred_test,tlist,algo='mtnn_prr',plot_residual=do_residual)
            comm_perfs['jaccard']=jaccard_score(y_true=obs_test['species'],y_pred=pred_test['species'], average='micro')
        except:
            comm_perfs = None
        y_true = Ys_true_test.values
    ...
    if method=='norm_prr':
        ...
        filter_prr = (Ys_hat.rank(axis=1,method='min',ascending=False)<=data_test_in[1 ])*1
        Y_hat_prr = Ys_hat * filter_prr
        species_perfs = eval_species_perfs(Y_hat_prr, Ys_true_test, th_list=eps)
        ...
        try:
            comm_perfs = plot_community_pred(obs_test,pred_test,tlist,algo='mtnn_opth_norm_prr',plot_residual=do_residual)
            comm_perfs['jaccard']=jaccard_score(y_true=obs_test['species'],y_pred=pred_test['species'], average='micro')
        except:
            comm_perfs = None
        y_true = Ys_true_test.values
    ...
    return prediction, species_perfs, comm_perfs, jaccard_perfs


def evaluate_pfal(rich_test, P_train, P_test, Y_train, Y_test, splist, tlist, T_scaled, method='raw', do_residual=False, eps=1e-6):
    ...
    ### Test prediction
    rich, cwm, fdis, _, _ = compute_functional_indices(Y_test,T_scaled)
    obs_test = {
            'species': Y_test,
            'cwm': cwm,
            'fdis': fdis,
    ...
    if method=='raw':
        pred_test['richness']= Ys_hat_test.sum(axis=1).values.reshape(-1,1)
        rich, cwm, fdis, _, _ = compute_functional_indices(Ys_hat_test,T_scaled)
        pred_test = {
            'species': Ys_hat_test,
            'cwm': cwm,
            'fdis': fdis,
    ...
    if method=='opth_norm':
        ...
        try:
            comm_perfs = plot_community_pred(obs_test,pred_test,tlist,algo='sdm_opth_norm',plot_residual=do_residual)
            comm_perfs['jaccard']=jaccard_score(y_true=obs_test['species'],y_pred=pred_test['species'], average='micro')
        except:
            comm_perfs = None
        y_true = Ys_true_test.values
    ...
    if method=='norm_prr':
        ...
        filter_prr = (Ys_hat.rank(axis=1,method='min',ascending=False)<=rich_test)*1
        Y_hat_prr = Ys_hat * filter_prr
        species_perfs = eval_species_perfs(Y_hat_prr, Ys_true_test, th_list=eps)
        ...
        try:
            comm_perfs = plot_community_pred(obs_test,pred_test,tlist,algo='sdm_opth_norm_prr',plot_residual=do_residual)
            comm_perfs['jaccard']=jaccard_score(y_true=obs_test['species'],y_pred=pred_test['species'], average='micro')
        except:
            comm_perfs = None
        y_true = Ys_true_test.values
    ...
    return comm_perfs, jaccard_perfs