...
class WeightedMoments(layers.Layer):
    ...
    def build(self, input_shape):
        self.funcdiv_layer = layers.Lambda(lambda x: tf.nn.weighted_moments(x[0], axes=[1],keepdims=False, 
                                                                            frequency_weights=x[1], name='weighted_moments'))
    ...
    def call(self, inputs):
        ...
        species_trait = tf.constant(self.trait_matrix,dtype=tf.float32, name='trait_constant')
        cm, cwvar = self.funcdiv_layer([species_trait,weights])
        cstd = tf.sqrt(cwvar)
...
class MTNN(tfk.Model):
    ...
    def build(self, input_shape):
        self.mlp_block = MLPBlock(hidden_units=self.hidden_units, activation=self.hidden_activation, 
                                  dropout_rate=self.dropout_rate, l1=0.0, l2=0.0, name='hidden')
        
        self.classifier = FCBlock(units=self.num_species, activation=self.out_activation, 
                                  dropout_rate=0.0, l1=self.l1, l2=self.l2, name='species_output')
        ...
        self.richness_layer = layers.Lambda(lambda x: tf.reduce_sum(x,keepdims=True,axis=1),
                                            name='richness',dtype=tf.float32) if self.optim_rich else None
    ...
    def call(self, inputs):
        ...
        if self.trait_matrix is not None:
            if self.apply_prr:
                ...
                traitcm, traitstd = self.funcdiv_layer(species_prr)
            else:
                traitcm, traitstd = self.funcdiv_layer(species_output)
                
            out_layers.update({'cm':traitcm, 'cstd': traitstd})
